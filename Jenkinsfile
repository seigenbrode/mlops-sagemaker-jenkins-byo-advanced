    ...
    stages {
      ...
      stage("BaselineModel") {
            steps { 
              script {
                def response = sh """ 
                aws lambda invoke --function-name ${params.LAMBDA_BASELINE_MODEL} --cli-binary-format raw-in-base64-out --region us-east-1 --payload '{"JobName": "'${params.SAGEMAKER_TRAINING_JOB}-${env.BUILD_ID}'", "Body": {"Payload": {"S3TrainingData": "${params.S3_TRAIN_DATA}", "InstanceCount": 1,"InstanceType": "ml.m5.xlarge","VolumeSizeInGB":15, "ImageUri": "${params.MMBASELINE_ECRURI}","S3OutputUri": "${params.SAGEMAKER_MM_BUCKET}/baseline","RoleArn": "${params.SAGEMAKER_EXECUTION_ROLE_TEST}"}}}' baselineresponse.json
              """
              }
            }
        }
            ...
            steps { 
              sh """
               if ! aws cloudformation describe-stacks --region us-east-1 --stack-name '${params.SAGEMAKER_TRAINING_JOB}'-prod ; then
                  echo -e "\nStack does not exist, creating ..."
                  aws cloudformation create-stack --region us-east-1 --stack-name '${params.SAGEMAKER_TRAINING_JOB}'-prod --template-body file://deploy/cfn-sagemaker-endpoint-prod.yml --parameters  ParameterKey=ModelName,ParameterValue=\"${params.SAGEMAKER_TRAINING_JOB}-${env.BUILD_ID}\" ParameterKey=ModelDataUrl,ParameterValue=\"${S3_MODEL_ARTIFACTS}/${params.SAGEMAKER_TRAINING_JOB}-${env.BUILD_ID}\"/output/model.tar.gz ParameterKey=TrainingImage,ParameterValue="${params.ECRURI}:${env.BUILD_ID}" ParameterKey=InstanceType,ParameterValue='ml.t2.large'  ParameterKey=InstanceCount,ParameterValue='1' ParameterKey=RoleArn,ParameterValue="${params.SAGEMAKER_EXECUTION_ROLE_TEST}" ParameterKey=Environment,ParameterValue='Prod' ParameterKey=DataCaptureS3Bucket,ParameterValue="${params.SAGEMAKER_MM_BUCKET}/data-capture-prod" ParameterKey=TrainingJobName,ParameterValue="${params.SAGEMAKER_TRAINING_JOB}-${env.BUILD_ID}"
                  echo "Waiting for stack to be created ..."
                  aws cloudformation wait stack-create-complete --region us-east-1 --stack-name "${params.SAGEMAKER_TRAINING_JOB}"-prod
               else
                  echo -e '\nStack exists, attempting update ...'
                  set +e
                  update_output=`aws cloudformation update-stack --region us-east-1 --stack-name '${params.SAGEMAKER_TRAINING_JOB}'-prod --template-body file://deploy/cfn-sagemaker-endpoint-prod.yml --parameters  ParameterKey=ModelName,ParameterValue=\"${params.SAGEMAKER_TRAINING_JOB}-${env.BUILD_ID}\" ParameterKey=ModelDataUrl,ParameterValue=\"${S3_MODEL_ARTIFACTS}/${params.SAGEMAKER_TRAINING_JOB}-${env.BUILD_ID}\"/output/model.tar.gz ParameterKey=TrainingImage,ParameterValue="${params.ECRURI}:${env.BUILD_ID}" ParameterKey=InstanceType,ParameterValue='ml.t2.large'  ParameterKey=InstanceCount,ParameterValue='1' ParameterKey=RoleArn,ParameterValue="${params.SAGEMAKER_EXECUTION_ROLE_TEST}" ParameterKey=Environment,ParameterValue='Prod' ParameterKey=DataCaptureS3Bucket,ParameterValue="${params.SAGEMAKER_MM_BUCKET}/data-capture-prod" ParameterKey=TrainingJobName,ParameterValue="${params.SAGEMAKER_TRAINING_JOB}-${env.BUILD_ID}"`
                  status=\$?
                  set -e
                  echo \$update_output
                  if [ \$status -ne 0 ] ; then
